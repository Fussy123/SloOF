        ...
        <div class="product">
            <div class="product_container">
                <div class="product_input">
                    <!-- Форма регистрации-->
                    <div class="product_text">Регистрация баннера </div>
                    <form class="product_form" action="" method="post" enctype="multipart/form-data">
                        ...
                        <input class="input_product" type="link" placeholder="ссылка на товар" name="link">
                        <input type="hidden" name="form" value="product">
                        <button class="button_product" type="submit" va;ie> Создать товар </button>
                    </form>
                </div>
                        ...
                        if ($_POST['form'] === 'product') {
                            ...
                            if (
                                ...
                                if ($uploadOk == 0) {
                                    ...
                                    if (empty($link)) {
                                        echo "Вы не вставилку ссылку";
                                    
                                    } else {
                                        $uniqid = uniqid("file_");
                                        if (move_uploaded_file($_FILES['product_image']['tmp_name'], $target_dir . $uniqid . "." . "png")) {
                                            $img = $target_dir . $uniqid . "." . "png";
                                            $stmt = $conn->prepare("INSERT INTO `banner` (img, link, login) VALUES (?, ?, ?)");
                                            $stmt->bind_param('sss', $img, $link, $login);
                                            $stmt->execute();

                                            echo 'Баннер загружен.';
                                        } else {
                                            echo'Баннер не загружен по какой то ошибке...';
                                        }
                                    }
                                }
                            }
